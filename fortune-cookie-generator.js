...
var generateFortuneCookie = function() {
    ...
    // `innerHTML` of the `#fortune-cookie-text` element
  
  var NewFortune = fortunesList[Math.floor(Math.random() * fortunesList.length)];
  
    document.getElementById("fortune-cookie-text").innerHTML = NewFortune;
  ...
  var PreviousFortune = document.createTextNode(NewFortune);  
  
  
  // 3. Select the `#previous-fortunes-container` container and use
  //    `appendChild()` to append the new `li` element you created above.
      
 PreviousList.appendChild(PreviousFortune);
  
 document.getElementById("previous-fortunes-container").appendChild(PreviousList);
